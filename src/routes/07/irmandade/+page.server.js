...
export const actions = {
    guerreiros: async ({ request }) => { 
        ...
        if (idade < 568036800000) return fail(400, { error: 'Precisa ter ao menos 18 anos!'});
    ...
    artesaos: async ({ request }) => { 
        ...
        if (destreza < 6) return fail(400,{ error: 'Precisa praticar mais habilidades manuais!'});
        if (ferramentas !='on') return fail(400, {error: 'Precisa de ferramentas!'});

        const atual = new Date(), aniversario = new Date(nascimento);
        const idade = atual - aniversario;
        if (idade < 473364000000) return fail(400,{ error: 'Precisa ter ao menos 15 anos!'});
